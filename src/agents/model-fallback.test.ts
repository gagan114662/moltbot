...
describe("runWithModelFallback", () => {
  ...
  it("filters fallback candidates using resilience provider allowlist", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    expect(result.result).toBe("ok");
    expect(run).toHaveBeenCalledTimes(2);
    expect(run.mock.calls[1]).toEqual(["google", "gemini-2.5-flash"]);
  });

  it("includes break-glass model as final fallback candidate", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
